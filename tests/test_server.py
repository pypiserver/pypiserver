...
def run_server(
    ...
    htpasswd = (
        CURRENT_PATH.joinpath("../fixtures/htpasswd.a.a")
        .expanduser()
        .resolve()
    )
    pswd_opt_choices = {
...
def wheel_file(project, tmp_path_factory):
    ...
    if re.match(r"^3\.7", sys.version):
        ...
        flags = f"--wheel --no-isolation --outdir {distdir}"
        assert run_py_build(project, flags) == 0
...
def run_twine(
    ...
    cmd = (
        f"twine {command} --repository test "
        f"--config-file {conf} {package}"
    )
...
def test_hash_algos(server_root, pipdir, hash_algo):
    ...
    cli = "--hash-algo {}".format(hash_algo)
    with run_server(server_root, other_cli=cli) as srv:
...
def test_twine_upload_json_upload_times_drive_uv_exclude_newer(tmp_path):
    ...
    assert (
        stored_upload_times[older_wheel.name]
        < stored_upload_times[newer_wheel.name]
    )
    ...
    with run_server(server_root, other_cli="--disable-fallback") as install_srv:
        ...
        files_by_version = {
            older_version: next(
                ...
                if item["filename"] == older_wheel.name
            ),
            newer_version: next(
                item
                for item in body["files"]
                if item["filename"] == newer_wheel.name
            ),
        }
        assert (
            files_by_version[older_version]["upload-time"]
            == stored_upload_times[older_wheel.name]
        )
        assert (
            files_by_version[newer_version]["upload-time"]
            == stored_upload_times[newer_wheel.name]
        )
        older_time = parse_upload_time(
            files_by_version[older_version]["upload-time"]
        )
        newer_time = parse_upload_time(
            files_by_version[newer_version]["upload-time"]
        )
        ...
        loose_cutoff = (newer_time + timedelta(seconds=1)).isoformat()
        ...
        loose_env = tmp_path / "loose-env"
        install_with_uv(
            loose_env,
            package,
            index_url,
            allow_host,
            exclude_newer=loose_cutoff,
        )
        assert installed_version(loose_env, package) == newer_version
