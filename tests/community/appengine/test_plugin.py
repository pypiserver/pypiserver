...
# Local imports
from pypiserver.community.appengine.plugin import (
...
def test_SynchronizerPlugin_sync_data_before_change_pulls_remote_files(
...
def test_SynchronizerPlugin_sync_data_before_change_stores_latest_local_files(
    storage_client: MagicMock,
):
    plugin = _get_syncronizer_plugin(storage_client)
    plugin.sync_data_before_change()
...
def test_SynchronizerPlugin_sync_data_after_change_gets_all_changes(
    storage_client: MagicMock,
):
    plugin = _get_syncronizer_plugin(storage_client)
...
def test_SynchronizerPlugin_sync_data_after_change_processes_all_changes(
    storage_client: MagicMock,
):
