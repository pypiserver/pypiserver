...
class SimpleFileBackend(Backend):
    ...
    def remove_package(self, pkg: PkgFile) -> None:
        if pkg.fn is not None:
            ...
            assert pkg.relfn is not None
            root = Path(pkg.root)
            upload_times = load_upload_times(root)
            relfn = normalize_upload_time_relfn(pkg.relfn)
            upload_times.pop(relfn, None)
            save_upload_times(root, upload_times)
...
class BackendProxy(IBackend):
    ...
    def add_package(self, filename: str, stream: t.BinaryIO) -> None:
        assert "/" not in filename
        return self.backend.add_package(filename, stream)
