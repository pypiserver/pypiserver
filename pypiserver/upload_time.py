...
def normalize_upload_time_relfn(relfn: str) -> str:
    return relfn.replace("\\", "/")
...
def load_upload_times(root: Path) -> dict[str, datetime]:
    ...
    try:
        ...
        for key, value in raw.items():
            normalized_key = normalize_upload_time_relfn(str(key))
            parsed[normalized_key] = parse_upload_time(str(value))
        return parsed
